/* ===========================================================================
   ...
   ===========================================================================
    ...
    CONTENT:    Maze workspace application
    VERSION:    2.4
   ===========================================================================
   ...
   ===========================================================================
    ...
    20-Nov-00 17:27:31  jp      last modification
    27-aug-99 17:16:22  gh      created
=========================================================================== */
/*
...
#if defined(WIN32) || defined(_LINUX_SOURCE)
#define fsin sin
#define fcos cos
#endif
...
#include "mazes.h"
#include "maze.h"
#define GRAVITATIONAL_CONSTANT 30.0
...
SbVec2f oneVector;
SoTransform *pipMazeXf;
SoTransform *trackerXf=NULL;
...
SoTimerSensor *timer=NULL;
SoAlarmSensor *resetTimer=NULL;
SoFieldSensor *transformSensor=NULL;
...
void
computeFloorTilt( void * data, SoSensor * )

//
////////////////////////////////////////////////////////////////////////
{
	SoTransform * trackerXf = (SoTransform *) data;

	SbVec3f xXfVec, zXfVec;
	float x=0,y=0,z=0;


    // Compute X and Z rotation angles depending on the mouse motion
	trackerXf->rotation.getValue().multVec(SbVec3f(1,0,0), xXfVec);
	xXfVec.getValue(x,y,z);
    mouseRotation[1] = (1.57 - (float) acos(y));
		
	trackerXf->rotation.getValue().multVec(SbVec3f(0,0,1), zXfVec);
	zXfVec.getValue(x,y,z);
    mouseRotation[0] = (float) (acos(y) - 1.57);
...
MAZE_API void Stb_init(int userID, StbCommunicator *cMan)
{
//    SoDB::setDelaySensorTimeout(SbTime(0.0));
    ...
    SoSeparator *mainRoot = new SoSeparator;
		...
		/* react to changes from pip */
		
	SoTrakEngine *pipEngine = cMan->getSoTrakEngine(
		cMan->getPipFromUser(userID));

	trackerXf = new SoTransform;
	trackerXf->ref();
	trackerXf->translation.connectFrom(&pipEngine->translation);
	trackerXf->rotation.connectFrom(&pipEngine->rotation);

	transformSensor = new SoFieldSensor(computeFloorTilt, 
		(void *) trackerXf);							
	transformSensor->attach(&trackerXf->translation);
   ...
   startRot->angle = 1.57;
	...
	pipMazeXf = new SoTransform;
	pipMazeXf->translation.setValue(0,0.1,0);

    mainRoot->ref();
   mainRoot->addChild(startRot);
	mainRoot->addChild(pipMazeXf);
    mainRoot->addChild(mazeRoot);

	SoTransform *startButtonXf = new SoTransform;
	startButtonXf->translation.setValue(7, 0.2, 0);

	So3DButton * startButton = new So3DButton;
	startButton->width = 2;
	startButton->height = 0.6;
	startButton->depth = 4;
	startButton->addPressCallback(startMazeCB);


	SbString filename = SbString(cMan->workingDirectory);
	filename+=SbString("start.rgb");
	SoTexture2 *buttonTexture = new SoTexture2;
	buttonTexture->filename = filename.getString();
	startButton->setupTexture(buttonTexture,0,0);

	mainRoot->addChild(startButtonXf);
	mainRoot->addChild(startButton);
	...
	if (cMan->getPipFromUser(userID) != -1)
		cMan->setPipGeometry(userID, mainRoot, FALSE);

}

MAZE_API SbBool Stb_exit(void *)
{
    if(timer) delete timer;
	if(resetTimer) delete resetTimer;
	if(transformSensor) delete transformSensor;
    if(trackerXf)
	{
	    trackerXf->translation.disconnect();
	    trackerXf->rotation.disconnect();
		trackerXf->unref();
	}	
	return TRUE;
}
