...
struct nv_struct { GLenum pname; unsigned int num_values; } num_values_array[] = {
...
#ifdef CR_EXT_secondary_color
	{ GL_CURRENT_SECONDARY_COLOR_EXT, 3 },
#endif /* CR_EXT_secondary_color */
...
{
	GLint *get_values;
	int tablesize = __numValues( pname ) * sizeof(GLint);
	(void) params;
...
{
	GLfloat *get_values;
	int tablesize = __numValues( pname ) * sizeof(GLfloat);
	(void) params;
...
{
	GLdouble *get_values;
	int tablesize = __numValues( pname ) * sizeof(GLdouble);
	(void) params;
...
void SERVER_DISPATCH_APIENTRY crServerDispatchGetBooleanv( GLenum pname, GLboolean *params )
{
	...
	int tablesize = __numValues( pname ) * sizeof(GLboolean);
	(void) params;
