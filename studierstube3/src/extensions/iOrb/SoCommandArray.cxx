 ...
 * @author  EMC
 *
 * $Id: SoCommandArray.cxx,v 1.12 2005/01/26 23:16:52 chris Exp $
 * @file                                                                   */
/* ======================================================================= */
...
#include "SoCommandArray.h"
#include "SoMenuManager.h"

SO_KIT_SOURCE(SoCommandArray);

void SoCommandArray::initClass(void) 
{
	if (SoType::fromName("SoCommandArray").isBad())
	{
		SO_KIT_INIT_CLASS(SoCommandArray, SoArrayWidget, "SoArrayWidget");
	}
}

SoCommandArray::SoCommandArray(void)
{
	...
	SO_KIT_CONSTRUCTOR(SoCommandArray);
	...
	itemsSensor.setFunction(SoCommandArray::itemsChanged);
	itemsSensor.attach(&items);
	labelsSensor.setData(this);
	labelsSensor.setFunction(SoCommandArray::labelsChanged);
...
SbBool SoCommandArray::setUpConnections(SbBool onOff, SbBool doItAlways)
{
...
void SoCommandArray::setRingRotation()
{
	transform->setMatrix(SbMatrix::identity());
}


void SoCommandArray::createGeometry()
{
	destroyGeometry();
	...
	for (int line = 0; line < displayLines.getValue(); line++)
	{
		...
		if (currentItem == i)
		{
			button = getTemplateGeometryHighLight();
		} else {
			button = getTemplateGeometry();
		}
...
void SoCommandArray::destroyGeometry()
{
...
void SoCommandArray::setCommand()
{
	if (currentItem > -1)
	{
...
void SoCommandArray::onClickSelect()
{
	// User clicks to select an item in the list
	setCommand();
}

void SoCommandArray::onClickConfirm()
{
	// User clicks to confirm selection
	setStartWidget();
}

void SoCommandArray::onPrimaryRotationChanged(float angleIncrement)
{
	// React on user input
...
void SoCommandArray::onSecondaryRotationChanged(float angleIncrement)
{
	// React on user input
}


void SoCommandArray::itemsChanged(void *data, SoSensor *sensor)
{
	assert(data != NULL);
	assert(((SoNode*)data)->isOfType(SoCommandArray::getClassTypeId()));

	SoCommandArray *self = (SoCommandArray*)data;

	if ((self->currentItem == -1) && (self->items.getNum() > 0)) { self->currentItem = 0; }
	self->createGeometry();
}

void SoCommandArray::labelsChanged(void *data, SoSensor *sensor)
{
	assert(data != NULL);
	assert(((SoNode*)data)->isOfType(SoCommandArray::getClassTypeId()));
