 ...
 * @author  EMC
 *
 * $Id: SoStringRing.cxx,v 1.13 2005/01/24 19:26:36 emcgames Exp $
 * @file                                                                   */
/* ======================================================================= */

#include <Inventor/nodes/SoMaterial.h>

#include "SoStringRing.h"
#include "SoMenuManager.h"

SO_KIT_SOURCE(SoStringRing);

void SoStringRing::initClass(void) 
{
	if (SoType::fromName("SoStringRing").isBad())
	{
		SO_KIT_INIT_CLASS(SoStringRing, SoRingWidget, "SoRingWidget");
	}
}

SoStringRing::SoStringRing(void)
{
	...
	SO_KIT_CONSTRUCTOR(SoStringRing);
	...
	itemsSensor.setFunction(SoStringRing::itemsChanged);
...
SbBool SoStringRing::setUpConnections(SbBool onOff, SbBool doItAlways)
{
...
void SoStringRing::createGeometry()
{
	destroyGeometry();
...
void SoStringRing::destroyGeometry()
{
...
void SoStringRing::onClickSelect()
{
...
void SoStringRing::onClickConfirm()
{
...
void SoStringRing::onPrimaryRotationChanged(float angleIncrement)
{
	// React on user input
...
void SoStringRing::onSecondaryRotationChanged(float angleIncrement)
{
	// React on user input
}

void SoStringRing::setRingRotation()
{
	transform->rotation.setValue(SbVec3f(0.0f, 0.0f, 1.0f), (float)((currentAngle*2*3.141592)/360.0f));
}

void SoStringRing::itemsChanged(void *data, SoSensor *sensor)
{
	assert(data != NULL);
	assert(((SoNode*)data)->isOfType(SoStringRing::getClassTypeId()));

	SoStringRing *self = (SoStringRing*)data;
