 ...
 * ======================================================================== */
/** SoBase3DSlider - implementation
  *
  * @author  Hermann Wurnig, Gerd Hesina
  *
  * $Id: SoBase3DSlider.cxx 3781 2004-09-28 11:19:51Z reitmayr $
  * @file                                                                   */
 /* ======================================================================= */
...
int  SoBase3DSlider::baseInitClassAttempts=0;

SO_KIT_SOURCE(SoBase3DSlider);

BASE3D_SOURCE(SoBase3DSlider);

void
SoBase3DSlider::getBoundingBox(SoGetBoundingBoxAction *action)
{
  int numIndices;
  const int *indices;
...
void 
SoBase3DSlider::setupSliderMesh(void *data, SoSensor *)
{
	int i;
	
	SoBase3DSlider *slider = (SoBase3DSlider *) data;
		
	float width = slider->width.getValue();
	float sWidth = 0.18*width;
	float height = slider->radius.getValue()*0.7;
	...
	bCube = new SoCube;
	bCube->width=width*1.15;
	bCube->height=height*1.5;
	bCube->depth=depth*1.15;
	...
	bCube = new SoCube;
	bCube->width = sWidth*1.15;
	bCube->height = height*1.5;
	bCube->depth = depth*1.15;
	slider->setSliderSurrounding(bCube);

	slider->startPoint.setValue(-0.4*width,0,0);
	slider->endPoint.setValue(+0.4*width,0,0);
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setAdditionalGeometry(SoNode *geometry)
...
/////////////////////////////////////////////////////////////////////
SoNode *
SoBase3DSlider::getAdditionalGeometry()
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setBodySurrounding(SoNode *newBody)
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setSliderSurrounding(SoNode *newSlider)
{		// replace old with new node
...
/////////////////////////////////////////////////////////////////////
SoNode *
SoBase3DSlider::getBodySurrounding()
...
/////////////////////////////////////////////////////////////////////
SoNode *
SoBase3DSlider::getSliderSurrounding()
{		// find node and return it
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setBodyGeometry(SoNode *newBody, SoSensorCB *callback)
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setSliderGeometry(SoNode *newSlider)
{		// replace old with new node
...
/////////////////////////////////////////////////////////////////////
SoNode *
SoBase3DSlider::getBodyGeometry()
{		// find node and return it
...
/////////////////////////////////////////////////////////////////////
SoNode *
SoBase3DSlider::getSliderGeometry()
{		// find node and return it
...
/////////////////////////////////////////////////////////////////////
void
SoBase3DSlider::disableHighlighting(SbBool disHigh)
{
...
/////////////////////////////////////////////////////////////////////
SbBool 
SoBase3DSlider::isSliderHighlighted()
{
...
/////////////////////////////////////////////////////////////////////
// Name:		isSliderHighlighted
// Purpose:		returns FALSE, if slider is not highlighted by 
//				the given station, else TRUE
// Parameters:	int station
// Returnvalue: FALSE, when not highlighted by station, else TRUE
/////////////////////////////////////////////////////////////////////
SbBool 
SoBase3DSlider::isSliderHighlighted(int station)
{
    return (sliderHLArray.find( station ) != sliderHLArray.end());
}
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setSliderHighlighted(SbBool highlightB, int station)
{
	if (highlightB == TRUE)
	{
        sliderHLArray.insert( station );
	
		if ((dragging == FALSE) && (sliderHL.getValue() == 0))
			sliderHL.setValue(1);
	}
	else
	{
        sliderHLArray.erase( station );
        if( sliderHLArray.empty() && ( dragging == FALSE ))
			sliderHL.setValue(0);		
	}
}
...
/////////////////////////////////////////////////////////////////////
SbBool 
SoBase3DSlider::isBodyHighlighted()
{
...
/////////////////////////////////////////////////////////////////////
// Name:		isBodyHighlighted
// Purpose:		returns FALSE, if body is not highlighted by 
//				the given station, else TRUE
// Parameters:	int station
// Returnvalue: FALSE, when not highlighted by station, else TRUE
/////////////////////////////////////////////////////////////////////
SbBool 
SoBase3DSlider::isBodyHighlighted(int station)
{
    return (bodyHLArray.find( station ) != bodyHLArray.end());
}
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::setBodyHighlighted(SbBool highlightB, int station)
{		// only change value when necessary
	if (highlightB == TRUE)
	{
        bodyHLArray.insert( station );
	
		if ((dragging == FALSE) && (bodyHL.getValue() == 0))
			bodyHL.setValue(1);
	}
	else
	{
        bodyHLArray.erase( station );
        if( dragging == FALSE )
            bodyHL.setValue( !bodyHLArray.empty());
	}
}
...
/////////////////////////////////////////////////////////////////////
void
SoBase3DSlider::setPosition(float )
{
}

void
SoBase3DSlider::setPositionUpdate(float )
{
	fprintf(stderr,"ATTENTION: SoBase3DSlider::abstract setPositionUpdate called!!! Please fix\n");
	assert(0);
}

float
SoBase3DSlider::getNormalizedIncrement(void)
{
	return 0;
}
...
{
	if (SoBase3DSlider::baseInitClassAttempts==0)
	{
		SoBase3DSlider::baseInitClassAttempts=1;

		// use initialization-macro
		SO_KIT_INIT_CLASS(SoBase3DSlider, SoBaseKit, "BaseKit");
	
		BASE3D_INIT_CLASS(SoBase3DSlider);
	}
}
...
/////////////////////////////////////////////////////////////////////
void
SoBase3DSlider::createSliderGraph()
{
...
/////////////////////////////////////////////////////////////////////
void
SoBase3DSlider::initFields(float width, float radius, float pos)
{
	if (pos > 1) pos = 1; 
	else if (pos < 0) pos = 0;
 	
	SO_KIT_CONSTRUCTOR(SoBase3DSlider);
	...
	SO_KIT_ADD_FIELD(startPoint,	(-0.4*width,0,0));
	SO_KIT_ADD_FIELD(endPoint,		(+0.4*width,0,0));
...
/////////////////////////////////////////////////////////////////////
// Name:		SoBase3DSlider
// Purpose:		Default-Constructor of class SoBase3DSlider
// Parameters:	---
// Returnvalue: ---
/////////////////////////////////////////////////////////////////////
SoBase3DSlider::SoBase3DSlider()
{
    firstTime=TRUE;
	initFields(50,3,0);
...
/////////////////////////////////////////////////////////////////////
SbBool
SoBase3DSlider::isInterested(SoHandle3DEventAction *h3a)
{
...
// 	SbBox3f box=bba->getBoundingBox();
	...
	{		// handle3DEvent has to be called for slider
		setSliderHighlighted(TRUE, ev->getStation());
		h3a->setGrabber(this);
		h3a->setHandled();
        ENABLEDIVNODE(this);
		return TRUE;
		...
		// now go on to body of slider
		// get path to slider
	SoPath *bodyPath = rootPath->copy();
	bodyPath->ref();
	bodyPath->append(kitSeparator.getValue());
	bodyPath->append(topSeparator.getValue());
	bodyPath->append(bodySeparator.getValue());

		// get boundingBox of body
	static SoGetBoundingBoxAction *bba3=new SoGetBoundingBoxAction(vp);
	bba3->apply(bodyPath);

	SbXfBox3f xfbox2=bba3->getXfBoundingBox();

	bodyPath->unref();
		// if pen insider
	if(xfbox2.intersect(ev->getTranslation())==TRUE)
	{		// then handle3DEvent has to be called for body of slider
		setBodyHighlighted(TRUE, ev->getStation());
		h3a->setGrabber(this);
		h3a->setHandled();
    	ENABLEDIVNODE(this);
		return TRUE;
	}
		// else event is not interesting for slider
...
/////////////////////////////////////////////////////////////////////
void 
SoBase3DSlider::handle3DEvent(SoHandle3DEventAction *h3a)
{
	DISABLEDIVNODE(this);
	So3DEvent *ev = (So3DEvent*) h3a->getEvent();
	    ...
	    ((dragging == TRUE) ) && (dragStation != ev->getStation()))
	{
		...
		{
			setSliderHighlighted(TRUE, ev->getStation());
			setBodyHighlighted(FALSE, ev->getStation());
		}
		else
		{
				// now go on to body of slider
			...
			if  (xfbox2.intersect(ev->getTranslation())==TRUE)
			{
				setSliderHighlighted(FALSE, ev->getStation());
				setBodyHighlighted(TRUE, ev->getStation());
			}
			else
			{
	...
	{
			// when event is button-release-event then 
		...
		{		// switch off dragging
			...
			dragging = FALSE;
            ENABLEDIVNODE(this);
    		setPositionUpdate(value+dragDistance);
			return;
		}	
		setPosition(value+dragDistance);
        ENABLEDIVNODE(this);
		return;
	}
	...
	if (dragging == FALSE)
	{
		if (isBodyHighlighted(ev->getStation()) == TRUE)
		{
			if ((ev->getType() == triggerButton.getValue()+1) &&
			...
			{	
				...
				// and set slider to its new position
				float incr = getNormalizedIncrement();
                ENABLEDIVNODE(this);
				if ((incr == 0) ||
					(fabs(position.getValue() - value) < incr))
						setPositionUpdate(value);
				else
				{
					if (position.getValue() > value) 
						setPositionUpdate(position.getValue()-incr);
					else
						setPositionUpdate(position.getValue()+incr);
				}
			}
		...
		else
		{
				...
				(ev->getButton(triggerButton.getValue()) == TRUE))
			{
				...
				dragDistance = position.getValue() - value;
...
void
SoBase3DSlider::updateFromNetwork(float newValue)
{
	fprintf(stderr,"ATTENTION: abstract SoBase3DSlider::updateFromNetwork called!!! Please fix\n");
	assert(0);
}
