...
int ACE_RMCast_Membership::ack (ACE_RMCast::Ack &ack)
{
    ...
    ACE_RMCast::Ack next_ack;
    {
        ...
        /*
        else if (ack.next_expected == this->next_expected_ && ack.highest_received == this->highest_received_ && ack.highest_received != 0)
        {
...
int ACE_RMCast_Membership::join (ACE_RMCast::Join &join)
{
    if (join.source == 0)
        return 0;
...
int ACE_RMCast_Membership::leave (ACE_RMCast::Leave &leave)
{
    if (leave.source == 0)
        return 0;
...
int ACE_RMCast_Membership::generate_ack (ACE_RMCast_Proxy *proxy)
{
    ACE_RMCast::Ack next_ack;
    {
        ACE_GUARD_RETURN (ACE_SYNCH_MUTEX, ace_mon, this->mutex_, -1);
...
int ACE_RMCast_Membership::compute_ack_i (ACE_RMCast_Proxy *source,
                                          ACE_RMCast::Ack &next_ack)
{
    // current time to eliminate proxies that timeout
    ...
    // throw out all old proxies....
    while( i != end )
    {
        ACE_Time_Value old((*i)->getLastTimeStamp());
        ACE_DEBUG((LM_DEBUG,"Membership: proxy %p with time %ld\n", (void*)(*i), old.sec()));
        if( (now - old) > timeout )
        {
            printf("Membership::removing old proxy %p with %ld - now %ld\n", (void*)(*i), old.sec(), now.sec());
    ...
    end = this->proxies_.end();
    i = this->proxies_.begin();
    ...
    for (; i != end; ++i)
    {
        ...
        /*
        ACE_Time_Value old = (*i)->getLastTimeStamp();
        ACE_DEBUG((LM_DEBUG,"Membership: proxy %x with time %d\n", (*i), old.sec()));
        if( (now - old) > timeout )
        {
            ACE_DEBUG((LM_DEBUG,"Membership::removing old proxy %x with %d - now %d\n", (*i), old.sec(), now.sec()));
            Proxy_Iterator cur = i;
            ++i;
            (void) this->proxies_.remove ((*cur));
            continue;
        }
        */  
        ...
        if (r < highest_received)  // important !!!! changed direction to get smallest hole
            highest_received = r;
    }
#if 0
...
int ACE_RMCast_Membership::close (void)
{
