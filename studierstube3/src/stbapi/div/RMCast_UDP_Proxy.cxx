                                            ...
                                            , peer_addr_ (addr)                                            
{
    ...
    ACE_DEBUG((LM_DEBUG,"Proxy::constructor %x - %d\n", this, last_time.sec()));
}

ACE_RMCast_UDP_Proxy::~ACE_RMCast_UDP_Proxy (void)
{
    ACE_DEBUG((LM_DEBUG,"Proxy::destructor %x - %d\n", this, last_time.sec()));
}

int
ACE_RMCast_UDP_Proxy::receive_message (char *buffer, size_t size)
{
    // timestamp reception of message
    this->last_time = ACE_OS::gettimeofday();
    ACE_DEBUG((LM_DEBUG,"Proxy::receive %x - %d\n", this, last_time.sec()));
    ...
    else if (type == ACE_RMCast::MT_DATA)
    {
       ...
       ACE_DEBUG((LM_DEBUG,"UDP_Proxy::receive data # %d, size %d, offset %d\n", 
           data.sequence_number, data.total_size, data.fragment_offset ));
