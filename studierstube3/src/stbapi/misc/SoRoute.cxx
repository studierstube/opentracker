...
/** SoRoute.cxx  - implementation of class SoRouter (version 2).
  ...
  * @author  Thomas Psik
  *
  * $Id: SoRoute.cxx 3964 2004-10-19 13:43:28Z reitmayr $
  * @file                                                                   */
 /* ======================================================================= */
...
SbBool SoRoute::updateFromFieldPtr(SoFieldContainer *fromNode, SbName fieldName, SoInput *in)
{
	...
	if (fromField == NULL)
	{
		displayError(in,"SoRoute::updateFromFieldPtr could not resolve field \"%s\" in \"%s\" node",
...
SbBool SoRoute::updateFromEngineOutput(SoEngine *fromEngine, SbName engineOutputName, SoInput *in)
{
	...
	displayError(in,"SoRoute::updateFromEngineOutput could not resolve EngineOutput or EngineField \"%s\" in \"%s\" engine.",
...
SbBool SoRoute::updateFromNodeEngineOutput(SoNodeEngine *fromNodeEngine, SbName engineOutputName, SoInput *in)
{
	...
	displayError(in,"SoRoute::updateFromNodeEngineOutput could not resolve EngineOutput or EngineField \"%s\" in \"%s\" node engine.",
		engineOutputName.getString(), fromNodeEngine->getName().getString());		
	return FALSE;
}


void SoRoute::displayError(const SoInput *in, const char *formatString ...)
{
    char	buf[10000];
    va_list	ap;
...
SbBool SoRoute::updateFromPtr(SoInput *in)
{
	...
	{
		displayError(in,"SoRoute::updateFromPtr from is empty.");		
		return FALSE;
	}
	...
	if (in != NULL) 
	{
		...
		if (fromBase  == NULL) 
		{
			displayError(in,"Unknown reference \"%s\"", firstPart.getString());		
			return FALSE;
		}
		...
		{
			displayError(in,"fromBase \"%s\"is neither an SoEngine nor a SoFieldContainer",firstPart.getString());		
			ret = FALSE;
		}
	...
	if (tempNode == NULL) 
	{
		...
		if (fromEngine == NULL) 
		{
			displayError(in, "SoRoute::updateFromPtr could not resolve field \"%s\", because no \"%s\" node/engine was found.",
...
SbBool SoRoute::updateToPtr(SoInput *in)
{
	...
	if (in != NULL) 
	{
		...
		if (toContainer  == NULL) 
		{
			displayError(in, "Unknown reference \"%s\"",
					  firstPart.getString());
			return FALSE;
		}
	}
	...
	if (toContainer == NULL) 
	{
		displayError(in, "SoRoute::updateToPtr could not resolve to field  \"%s\", because no \"%s\" node/engine was found",
			secondPart.getString(), firstPart.getString());
		return FALSE;
	}
	...
	if (toField == NULL)
	{
		displayError(in, "SoRoute::updateToPtr could not resolve to field \"%s\" in \"%s\" node/engine",
			secondPart.getString(),firstPart.getString());
		return FALSE;
	}
	return TRUE;
