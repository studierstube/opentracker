 ...
 *
 * $Id: SoStringCalculator.cxx 3425 2004-07-14 12:09:17Z tamer $
 * @file
 */
/* ======================================================================== */
    ...
    if (whichField==&expression){
        ...
        for (i=0;i<expression.getNum();i++){
            ...
            if ((marker1=exp.find_first_not_of(" "))==exp.npos){
                printf("STRINGCALCULATOR: expression parse warning: empty expression\n");
                return;
            }
            ...
            if (!isOutputVar && !isTempVar) {
                printf("STRINGCALCULATOR: expression parse error: output or temp variable not found\n");
                return;
            }
            ...
            if ((marker1=exp.find("=",marker1+2))==exp.npos &&
                ...
                printf("STRINGCALCULATOR: expression parse error: missing right hand side\n");
                return;
            }
            do {
                ...
                if ((marker1=exp.find_first_not_of(" ",marker1+1))==marker2){
                    printf("STRINGCALCULATOR: expression parse error: syntax error\n");
                    return;
                }
...
bool SoStringCalculator::parseExpression(std::string exp,SbStringList *inputVar,SbIntList *index0,SbIntList *index1)
{
    ...
    if (!oneLetterVar && !twoLetterVar){
        printf("STRINGCALCULATOR: incorrect variable name: %c\n",exp[0]);//,exp[1]);
        return false;
    }
    ...
    if ((marker0=exp.find_first_not_of(" ",oneLetterVar ? 1 : 2))!=exp.npos){
        if (exp[marker0]!='(') {
            printf("STRINGCALCULATOR: syntax error: %s\n",exp.c_str());
            return false;
        }
        // number found
        if ((marker0=exp.find_first_not_of(" ",marker0+1))!=exp.npos &&
            ...
            printf("STRINGCALCULATOR: error parsing 1st substring index: %s\n",exp.c_str());
            return false;
        }
        if ((marker0=exp.find_first_not_of(" ",marker1+1))!=exp.npos &&
            ...
            printf("STRINGCALCULATOR: error parsing 2nd substring index: %s\n",exp.c_str());
            return false;
        }
    }
