 ...
 *
 * $Id: SoFanIn.cxx 4203 2005-05-13 10:05:22Z kaufmann $
 * @file                                                                   */
/* ======================================================================= */ 
...
#ifdef DEBUG
        SoDebugError::post("SoFanIn::setup",
            "Can't fan in field of type %s",
...
{
#ifdef DEBUG
    printf("SoFanIn::inputChanged() called! \n");
#endif
...
{
#ifdef DEBUG
    printf("SoFanIn::evaluate() called! \n");
#endif
...
SoField *SoFanIn::getNextUnconnectedField() const{
/*	const SoFieldData *  data = getFieldData();
	int num = data->getNumFields();
	printf("number of fields %d\n", num);
	SoField *curField = NULL;
	for (int i=0; i< num; i++)
	{
		curField = data->getField(this, i);
		if (!curField->isConnected())
			return curField;
	}
*/
  return SoFanIn::getNextUnconnectedFieldFromEngine(this);
}

// static verion 
SoField *SoFanIn::getNextUnconnectedFieldFromEngine(const SoEngine *engine) {
	assert(engine!=NULL);
	assert(engine->getTypeId().isDerivedFrom(SoTriggerAny::getClassTypeId()) || 
		   engine->getTypeId().isDerivedFrom(SoFanIn::getClassTypeId()) 
	);
	
	const SoFieldData *  data = engine->getFieldData();
	int num = data->getNumFields();
	//printf("number of fields %d\n", num);
	SoField *curField = NULL;
	for (int i=0; i< num; i++)
	{
		curField = data->getField(engine, i);
		if (!curField->isConnected())
			return curField;
	}
	return NULL;
