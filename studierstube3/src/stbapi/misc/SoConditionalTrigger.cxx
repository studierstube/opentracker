...
#include "SoConditionalTrigger.h"
...
void SoConditionalTrigger::inputChanged(SoField * whichField)
{
    if ((whichField == &token) | (whichField == &triggerAtInit))
    {
	    ...
	    if (localC) {
            if (!triggerNotified) {
#if 0
	            printf("%s: condition == TRUE -> notify trigger!\n",this->getName().getString());
#endif
                boolOut.enable(TRUE);
                trigger.enable(TRUE);
            ...
            if (tempBool) {
#if 0
	            printf("%s: condition == FALSE -> update boolOut!\n",this->getName().getString());
#endif
                tempBool = false;
                boolOut.enable(TRUE);
            }
...
#if 0
	    printf("%s: evaluate(). Token:%s ",this->getName().getString(),token.getValue().getString());
