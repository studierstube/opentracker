...
begin
  ...
  { 16bit sound }
  FSound1 := FSOUND_Sample_Load(FSOUND_FREE, '../../media/drumloop.wav', FSOUND_2D, 0, 0);
  if FSound1 = nil then
  begin
  ...
  { 8bit sound }
  FSound2 := FSOUND_Sample_Load(FSOUND_FREE, '../../media/jungle.wav', FSOUND_2D, 0, 0);
  if FSound2 = nil then
  begin
    MessageDlg('Failed to load jungle.wav'#13 + FMOD_ErrorString(FSOUND_GetError), mtError, [mbOk], 0);
    Unload;
    Exit;
  end;
