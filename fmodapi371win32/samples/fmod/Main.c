...
void SaveSettings()
{
    ...
    RegSetValueEx(key, "cdletter", 0, REG_SZ,    setting_cdletter, strlen(setting_cdletter) + 1);
...
signed char AddFileToSongList(char *name, char *displayname)
{
	...
	currsong = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0);
...
void LoadURL()
{
	if (DialogBox(g_hinst, MAKEINTRESOURCE(IDD_LOADURLDLG), mainhwnd, FMOD_LoadURLDlgProc))
    {
        ...
        {
		    int songid = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0);
		    if (songid >= MAXSONGS) 
		    {
			    MessageBox(0, "Error.  Unable to fit any more songs in.. please restart app", "Loading a song", MB_ICONHAND|MB_OK|MB_SYSTEMMODAL);
                return;
...
void LoadCD()
{
    ...
    if (setting_cdda)
    {
        ...
        */
        sprintf(tmp, "%s*!", setting_cdletter);
        ...
        {
	        songid = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0);
	        if (songid >= MAXSONGS) 
	        {
		        MessageBox(0, "Error.  Unable to fit any more songs in.. please restart app", "Loading a song", MB_ICONHAND|MB_OK|MB_SYSTEMMODAL);
                break;
        ...
        numsongs = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0);
        for (i=0;i < numsongs;i++)
        {
            if (song[i].cdtrack)
            {
...
{
    int songid;
    int numsongs = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0);
...
void DeleteSong(int songid)
{
    ...
    */
	...
	{
		songid = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0) - 1;
		if (songid < 0) 
        {
			songid = 0;
        }
...
long FAR PASCAL ProgressWindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
	{
        ...
        case WM_MOUSEMOVE:
        {
			int songid = SendMessage(songlist_hwnd, LB_GETCURSEL, 0, 0);
	...
	switch (msg)
	{
			    ...
			    if (songid != LB_ERR)
			    {
                    ...
                    else if (song[songid].cdtrack)
                    {
                        ...
                        else
                        {
                            if (song[songid].cdtrack == FSOUND_CD_GetTrack(cddevice))
                            {
                                FSOUND_CD_SetVolume(cddevice, SendMessage((HWND)lParam, TBM_GETPOS, 0, 0));
                            }
                        }
                    }
			    }
		...
		case WM_COMMAND :
		{
			switch (LOWORD(wParam))
			{
				...
				case IDC_CONFIG :
				{
                    ...
                    */
                    numsongs = SendMessage(songlist_hwnd, LB_GETCOUNT, 0, 0);
                    currsong = SendMessage(songlist_hwnd, LB_GETCURSEL, 0, 0);
                    for (i=0;i < numsongs;i++)
                    {
		...
		case WM_TIMER :
		{
            ...
            */
			...
			songid = SendMessage(songlist_hwnd, LB_GETCURSEL, 0, 0);
			if (songid >= 0)
			{
