...
//****************************************************************************
	...
	DWORD thread0;
	HANDLE hThread0 = CreateThread(NULL,0,DummyThreadProc,(LPVOID)1000,0,&thread0);

	DWORD thread1;
	HANDLE hThread1 = CreateThread(NULL,0,DummyThreadProc,(LPVOID) 950,0,&thread1);
...
DWORD WINAPI DummyThreadProc(LPVOID lpParameter)
{
	...
	while(true)
	{
		DWORD wait_result = graphManager->WaitForNextSample(INFINITE);
		if(wait_result == WAIT_OBJECT_0)
		{
			graphManager->CheckoutMemoryBuffer(&l_mbHandle, &l_pPixelBuffer);
			Sleep(delay); // do nothing...
			graphManager->CheckinMemoryBuffer(l_mbHandle);
		}
	}
	return(0);
}
...
void render(void)   
{
	...
	if(wait_result == WAIT_OBJECT_0)
	{
		...
		//graphManager->Lock();
		graphManager->CheckoutMemoryBuffer(&g_mbHandle, &g_pPixelBuffer);
		glBindTexture(GL_TEXTURE_2D, g_texid[CUBE_TEX]);
		...
		graphManager->CheckinMemoryBuffer(g_mbHandle);
		//graphManager->Unlock();
	}
