...
def omniidl_emitter(target, source, env):
	...
	stem = str(idl_file).split(os.sep)[-1][0:-4]
	header_file = stem + '.hh'
	skeleton_file = stem + 'SK.cc'
	idl_directory = idl_file[0:-len(idl_file.split(os.sep)[-1])-1]
...
def generate_omniidl_actions(source, target, env, for_signature):
	print "GENERATE_OMNIIDL_ACTIONS:: generating file"
	idl_file = str(target[0])
	skeleton_directory = idl_file[0:-len(idl_file.split(os.sep)[-1])-1]
	return 'omniidl -bpython -bcxx -Wbtp -C %s %s' % (skeleton_directory, source[0])


def generate(env):
#	print "OMNIIDL: GENERATING ENVIRONMENT"
