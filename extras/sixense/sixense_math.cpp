...
inline Vector3 Matrix3::getEulerAngles() {
	...
	float rad2deg = 180.0f/3.1415926;
	...
	if( fabs( A ) > 0.005 ) {
		r = atan2f( -_cols[2][0]/A, _cols[2][2]/A ); // atan2( D, C )
		h = atan2f( -_cols[0][1]/A, _cols[1][1]/A ); // atan2( F, E )
	} else {
		r = 0;
		h = atan2f( _cols[1][2], _cols[0][2] ); // atan2( F, E ) when B=0, D=1
	}
