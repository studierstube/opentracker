...
static IS_BOOL  processInterTraxDataRecord(InterSenseTrackerType *tracker, char *cmdbuf, int numChars);
static IS_BOOL  processSystemOutputListRecord(InterSenseTrackerType *tracker, char *cmdbuf,int numChars);
                                    ...
                                    char *cmdbuf, int numChars)
{
    ...
    int    int14bit;
    BYTE   lo, hi;
    unsigned char joystickByte;
    unsigned int  joystickInt;
                ...
                switch(tracker->station[station].outputList[i])
                {
                    ...
                    case 22:
                        if(tracker->state.outputFormat == IT_COM_SYS_BINARY)
                        {
                            if(numChars-pos >= 1)
                            {
                                memcpy((void *)&joystickByte, buffer, 1);
                            }
                        ...
                        {
                            if(sscanf(buffer, "%u", &joystickInt) != 1)
                            {
                                joystickInt = 0;
                            }
                            joystickByte = (unsigned char) joystickInt;
...
static IS_BOOL processSystemOutputListRecord(InterSenseTrackerType *tracker, char *cmdbuf, int numChars)
{
    ...
    int itComOuputListCodes[IT_COM_NUM_LIST_CODES] =
                         {0, 1, 2, 4, 5, 6, 7, 11, 18, 19, 20, 21, 22};
...
IS_BOOL itComUpdateOutputList(InterSenseTrackerType *tracker, DWORD stationNum)
{
    float startTime;
    int attempt;
...
IS_BOOL itComUpdateStationStat(InterSenseTrackerType *tracker, DWORD stationNum)
{
    float startTime;
    int attempt;
...
IS_BOOL itComUpdateIntrackSystemStat(InterSenseTrackerType *tracker, float timeOut)
{
    float startTime;
    int attempt;
...
IS_BOOL itComUpdateSensorStat(InterSenseTrackerType *tracker, DWORD stationNum)
{
    float startTime;
    int attempt;
...
IS_BOOL itComUpdatePredictionString(InterSenseTrackerType *tracker, DWORD stationNum)
{
    float startTime;
    int attempt;
...
/******************* itComUpdateImu *************************/
IS_BOOL itComUpdateImu(InterSenseTrackerType *tracker, DWORD stationNum)
{
    float startTime;
    int attempt;
...
IS_BOOL itComUpdateSensitivityVal(InterSenseTrackerType *tracker, DWORD stationNum)
{
    float startTime;
    int attempt;
